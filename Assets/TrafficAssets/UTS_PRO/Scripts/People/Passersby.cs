...
public class Passersby : MonoBehaviour
{
    ...
    [SerializeField] private SemaphorePeople nearSemaphore;
    ...
    //сортировка целей в зоне видимости
        ...
        foreach(T t in list)
        {
            ...
            if (Vector3.Angle(transform.forward, dirToTarget) < viewAngle / 2)
            {
                ...
                if (!Physics.Raycast(transform.position, dirToTarget, dstToTarget, obstacleMask))
                {
                    ...
                    {
                        //если человек ещё не видит светофор
                        if (nearSemaphore == null)
                        {
                            //человек сейчас видит т(светофор)
